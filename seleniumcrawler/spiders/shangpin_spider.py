...
class ShangPinSpider(scrapy.spiders.Spider):
    ...
    def start_requests(self):
        ...
        sql = "select `药品通用名` as drug_name, `国产药品数` as guochan_count, `进口药品数` as jinkou_count from `通用名药品` where loaded=0 limit 1000"
        ...
        for (drug_name, guochan_count, jinkou_count) in self.cursor:
            ...
            logging.info(u'DRUG %s finished. No：%s '%(drug_name, No) )
            No += 1
        # return requests
